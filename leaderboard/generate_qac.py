# Given QAC samples, generate new answers with candidate LLMs for eval and LB rankings
...
parser.add_argument("--seed_data", type=str, required=True, help="Path to the seed data file.")
parser.add_argument("--domain", type=str, required=True, help="Data domain. Example: IT support.")
parser.add_argument("--reference_model", default="gpt-4-1106-preview", type=str, help="Language model to use as reference for answer generation.")
parser.add_argument("--generators", type=list, help="List of language models to evaluate for answer generation.",
                    default=[
                              "gpt-4o",
                              "gpt-4o-mini",
...
def get_examples_from_csv(file_path):
  ...
  for index, row in data.iterrows():
    ...
    raw_contexts = []
    raw_contexts.append(row['Context'])
    qac_example.context = raw_contexts
    examples.examples.append(qac_example)
...
def write_json_output(output_file, gen_data_list, generators, instructions):
    ...
    for gen_data, generator_name in zip(gen_data_list, generators):
        for example, instruction in zip(gen_data.examples, instructions):
            data.append({
...
if __name__ == "__main__":
    ...
    seed_data.examples = seed_data.examples
    gen_data_list = []
    for generator_name in args.generators:
       generator_llm = llm_lib.LLM(generator_name)
       gen_data, instructions = write_answers_with_model(seed_data, generator_llm, args.domain)
       gen_data_list.append(gen_data)
    
    ref_llm = llm_lib.LLM(args.reference_model)

    reference_data, instructions = write_answers_with_model(seed_data, ref_llm, args.domain)

    # save in alpaca_eval format
    ...
    write_json_output(output_file, [reference_data], [args.reference_model], instructions)
