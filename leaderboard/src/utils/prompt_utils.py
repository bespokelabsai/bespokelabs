...
REQUIREMENTS = {
    ...
    "json_output_with_keys": Template(
        "Your output must be JSON, with keys: $keys_list in this exact order."
    ),
    "json_list_output_with_keys": Template(
        "Your output must be a list of JSONs, where each JSON has keys: $keys_list in this exact order."
    ),
    "just_output": Template("Just output $what_to_output and nothing else.").substitute(
        what_to_output="what I asked you to output in the format I asked you and nothing else."
    ),
    "many_different_options": Template(
        "Before giving me your final output, create a list of $num_trials potential outputs."
    ).substitute(num_trials=100),
...
def get_prompt(
    ...
    if examples:
        expert_prompt += (
            f"<EXAMPLES> Here are some examples:\n{examples}\n</EXAMPLES>\n"
        )
